...
> ⚠️ **Статус:** Phase 8 готова — пакет собирается и валиден для публикации. Готовы все команды: `sdd init` (+ `--industry fintech|healthcare|e-commerce`), `sdd brainstorm <topic>` (10 этапов), `sdd integrations <list|show|add|edit|remove|validate|import|spec>` (13 категорий + OpenAPI/AsyncAPI/BPMN импортёры), `sdd spec` (14 секций arc42/IEEE 29148, авто-Mermaid + валидация, `--update` diff-mode + `--format html|pdf|confluence` через pandoc), `sdd status`, `sdd add|edit|remove <topic>`, `sdd add feature|adr|risk|integration`, `sdd lint` (+`--strict`, `--warnings-as-errors`), `sdd import --from md|jira|linear`, `sdd migrate`. Глобальные опции `--verbose` / `--provider <cli\|api>`. Унифицированные exit codes 0/1/2/3/4 (success/generic/validation/provider/filesystem). Public API в `src/index.ts`. Тестовое покрытие: 45 suites / 222 tests / 3 snapshot'а; gates — domain ≥90%, adapters ≥75%, application ≥85%, utils ≥95%. `npm pack --dry-run` собирает `sdd-generator-0.1.0.tgz` (~93.5 kB, 223 файла). LICENSE / CONTRIBUTING / CHANGELOG (Keep a Changelog) на месте. Финальный шаг — `npm publish` + GitHub Release — оставлен на maintainer-а.
